...
class Params:
    ...
    resource_evidence_update_strength: float = 1.0
    resource_learning_strength: float = 1.0
    school_resource_coupling: float = 0.55
    ...
    credential_discount_strength: float = 2.0
    major_discount_strength: float = 1.3
    # student choice
...
def centered_resource_modifier(resource, midpoint, slope, strength):
    """Preserve the baseline mean at resource=0.5 while scaling only the resource gradient."""
    return midpoint + strength*slope*(resource-0.5)


def learning_modifier(resource, mode, p):
    if mode == 'generic':
        return centered_resource_modifier(resource, 0.775, 0.45, p.resource_learning_strength)
    if mode == 'explained':
        return centered_resource_modifier(resource, 0.750, 0.50, p.resource_learning_strength)
    if mode == 'lsm':
        return centered_resource_modifier(resource, 0.960, 0.08, p.resource_learning_strength)
    raise ValueError(mode)


def evidence_modifier(resource, p):
    return centered_resource_modifier(resource, 0.675, 0.65, p.resource_evidence_update_strength)
...
def make_base_society(seed:int, p:Params):
    ...
    school_signal=np.clip(0.275 + p.school_resource_coupling*(resource-0.5) + 0.15*stu_seniority/2 + rng.normal(0,0.18,p.n_students),0,1)
    ...
    for i in range(p.n_students):
        ...
        ev_base=0.455 + p.resource_evidence_coupling*(resource[i]-0.5) + 0.10*stu_seniority[i]
...
def employer_decision_score(state,i,j,regime,rng,p:Params):
    ...
    wcred=prior/(1.0+p.credential_discount_strength*evidence_conf)
    wmajor=0.12/(1.0+p.major_discount_strength*evidence_conf)
...
def learning_step(state,i,regime,rng,p,round_idx,events):
    ...
    realized_gain=0.0; evidence_gain=0.0; quality=np.nan
    ...
    if regime in ('R0','R1'):
        ...
        realized_gain=p.generic_learning_rate*quality*learning_modifier(resource,'generic',p)
        evidence_gain=p.evidence_selfstudy_gain*quality*evidence_modifier(resource,p)
    elif regime=='R2':
        ...
        if rng.random()<uptake:
            ...
            realized_gain=p.explained_learning_rate*quality*learning_modifier(resource,'explained',p)
            evidence_gain=p.evidence_selfstudy_gain*1.35*quality*evidence_modifier(resource,p)
        else:
        ...
        if rng.random()<uptake:
            ...
            if rng.random()<accept:
                ...
                realized_gain=p.lsm_learning_rate*quality*learning_modifier(resource,'lsm',p)
                ...
                quality=np.clip(rng.beta(3.0,2.5)*readiness,0,1)
                realized_gain=p.explained_learning_rate*0.70*quality*learning_modifier(resource,'explained',p)
                evidence_gain=p.evidence_selfstudy_gain*quality*evidence_modifier(resource,p)
        else:
            target_skill=int(rng.integers(0,p.n_skills)); effort=1.0
            quality=np.clip(rng.beta(3.0,2.5)*readiness,0,1)
    ...
    completed=bool(accepted and regime=='R3' and action=='LSM task' and quality >= 0.55)
    ...
    events.append(dict(round=round_idx,student=i,regime=regime,target_job=j,target_skill=target_skill,
                       attempted=attempted,accepted=accepted,completed=completed,action=action,resource=resource,quality=quality,
...
def credential_audit(state,regime,p):
    ...
    for i in range(p.n_students):
        ...
        wcred=prior/(1+p.credential_discount_strength*evidence_conf)
        wmajor=0.12/(1+p.major_discount_strength*evidence_conf)
...
def simulate(seed:int,regime:str,p:Params):
    ...
    q1_placement=float(employed[low].mean())
    q4_placement=float(employed[high].mean())
    opp_gap=float(q4_placement-q1_placement)
    ...
    return dict(seed=seed,regime=regime,placement_rate=float(employed.mean()),ground_truth_fit=float(np.nanmean(gtf_hires)),
                ...
                realized_gain=mean_gain,evidence_opportunity_gap=opp_gap,resource_q1_placement=q1_placement,
                resource_q4_placement=q4_placement), pd.DataFrame(round_rows), lev, pd.DataFrame(hire_events)
...
def summarize_run_metrics(df):
    ...
    metrics=['placement_rate','ground_truth_fit','search_rounds','applications_per_hire','fill_rate','vacancy_duration',
             'credential_sensitivity','investment_alignment','total_gap_reduction','evidence_conversion','task_acceptance','realized_gain','evidence_opportunity_gap','resource_q1_placement','resource_q4_placement']
    for reg in REGIMES:
...
def paired_contrasts(df):
    ...
    metrics=['placement_rate','ground_truth_fit','search_rounds','applications_per_hire','fill_rate','vacancy_duration',
             'credential_sensitivity','investment_alignment','total_gap_reduction','evidence_conversion','evidence_opportunity_gap','resource_q1_placement','resource_q4_placement']
    rows=[]
...
def run_experiment(n_seeds=40,p=Params(),label='main'):
...
if __name__=='__main__':
    p=Params()
    m,r,l,h=run_experiment(n_seeds=40,p=p,label='main')
    sensitivity()
    meta={
